...
@numba.njit
def reflected_int_single_point(
    R,
    zgrating,
    back_project_x,
    back_project_y,
    ...
    Rp = np.array(
        [
            R[0] - (R[2] - zgrating) * back_project_x,
...
@numba.njit
def input_int_single_point(
    R, zgrating, center_hole, cos_th_centers, sin_th_centers, r_stop, s_max, wb
):
    # Are we behind the grating?
    if R[2] > zgrating:
...
class infiniteGratingMOTBeams(laserBeams):
    ...
    def jones_vector(self, R=np.array([0.0, 0.0, 0.0]), t=0):
        output = []
    ...
    def stokes_parameters(self, R=np.array([0.0, 0.0, 0.0]), t=0):
        output = []
        for ii, beam in enumerate(self.beam_vector):
            if (ii % (self.nr + 1)) > 0:
                output.append(
    ...
    def polarization_ellipse(self, R=np.array([0.0, 0.0, 0.0]), t=0):
        output = []
        for ii, beam in enumerate(self.beam_vector):
            if (ii % (self.nr + 1)) > 0:
                output.append(
                    beam.polarization_ellipse(
                        self.svec[:, ii % (self.nr + 1) - 1],
                        self.pvec[:, ii % (self.nr + 1) - 1],
                        R=R,
                        t=t,
...
class inputGaussianBeam(clippedGaussianBeam):
    ...
    def intensity(self, R=np.array([0.0, 0.0, 0.0]), t=0.0):
        """
...
class reflectedGaussianBeam(clippedGaussianBeam):
    def __init__(
        self,
        kvec=np.array([-1 / np.sqrt(2), 0, -1 / np.sqrt(2)]),
        s=1.0,
        delta=-1.0,
        pol=np.array([-1 / np.sqrt(2), 1j / np.sqrt(2), 0]),
        pol_coord="cartesian",
        ...
        **kwargs,
        ...
        if not np.allclose(kvec_in[:2], 0.0):
            raise ValueError("inputGaussianBeam must be aligned with z axis")
    ...
    def back_project(self, R):
        if R.shape == (3,):
            Rp = np.array(
                [
                    R[0] - (R[2] - self.zgrating) * self.back_project_x,
        ...
        **kwargs,
        ...
        if eta0 is not None:
            ...
            if reflected_pol_basis == "poincare":
                ...
                raise NotImplementedError(
                    "Only Poincare basis is implemented for the zeroth order beam."
                )
