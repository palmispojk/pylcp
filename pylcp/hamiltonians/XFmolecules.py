...
def Xstate(
    ...
    q0=0,
    q2=0,
    gS=-cts.value("electron g factor"),
    gI=cts.value("proton g factor"),
    ...
    for ll, q_i in enumerate(qs):
        for ii, basis_i in enumerate(basis):
            for jj, basis_j in enumerate(basis):
                mu_p[ll, ii, jj] = electronspinzeeman(
                    basis_i["Lambda"],
                    basis_i["N"],
                    basis_i["J"],
                    basis_i["F"],
...
def Astate(
    ...
    def zeeman(L, S, J, O, I, F, mF, P, Lp, Sp, Jp, Op, Ip, Fp, mFp, Pp):
        ...
        reduced_matrix_elements += (
            ...
            * __wig3j(J, 1, Jp, -O, 0, Op)
            * np.sqrt((2 * F + 1) * (2 * Fp + 1) * (2 * J + 1) * (2 * Jp + 1))
            * (L == Lp)
            * (O == Op)
            * (S == Sp)
            * (I == Ip)
        ...
        reduced_matrix_elements += (
            ...
            * gN
            * (-1) ** (Fp + J + I + 1)
            * __wig6j(I, F, J, Fp, I, 1)
            * np.sqrt(I * (I + 1) * (2 * I + 1) * (2 * F + 1) * (2 * Fp + 1))
            * (L == Lp)
            * (S == Sp)
            * (J == Jp)
            * (O == Op)
        ...
        reduced_matrix_elements += (
            ...
            * (-1) ** (Fp + J + I + 1)
            * __wig6j(J, F, I, Fp, Jp, 1)
            * np.sqrt(J * (J + 1) * (2 * J + 1) * (2 * F + 1) * (2 * Fp + 1))
            * (L == Lp)
            * (S == Sp)
            * (J == Jp)
            * (O == Op)
        ...
        for kk, p in enumerate([-1, 0, 1]):
            mu_q[kk] = (
                ...
                * __wig3j(F, 1, Fp, -mF, p, mFp)
                * reduced_matrix_elements
            )  # check if we need the (-1)**p factor here!!!
        return mu_q
...
def dipoleXandAstates(
    ...
    def dipole_matrix_element(
        ...
        return (
            (-1) ** (F - mF)
            * __wig3j(F, 1, Fp, -mF, q, mFp)
            * (-1) ** (Fp + J + I + 1)
            * np.sqrt((2 * F + 1) * (2 * Fp + 1))
            * __wig6j(Jp, Fp, I, F, J, 1)
            * (-1) ** (J - O)
            * np.sqrt((2 * J + 1) * (2 * Jp + 1))
        ...
        for jj, intbasis_ba_i in enumerate(intbasis_ba):
            for kk, intbasis_ap_i in enumerate(intbasis_ap):
                intdijq[ii, jj, kk] = dipole_matrix_element(
                    intbasis_ba_i["Lambda"],
                    intbasis_ba_i["Sigma"],
                    intbasis_ba_i["Omega"],
                    intbasis_ba_i["J"],
                    intbasis_ba_i["F"],
                    intbasis_ba_i["mF"],
                    intbasis_ap_i["Lambda"],
                    intbasis_ap_i["Sigma"],
                    intbasis_ap_i["Omega"],
                    intbasis_ap_i["J"],
                    intbasis_ap_i["F"],
...
if __name__ == "__main__":
    ...
    for ii, B_i in enumerate(B):
        Es_X[ii, :], Us = np.linalg.eig(
            H0_X - Bq_X[0] / np.sqrt(2) * B_i + Bq_X[2] / np.sqrt(2) * B_i
        )
        Es_X[ii, :] = np.sort(Es_X[ii, :])
    ...
    # Let's see if I get the same thing putitng it in the x-direction:
    for ii, B_i in enumerate(B):
        Es_A[ii, :], Us = np.linalg.eig(
            H0_A - Bq_A[0] / np.sqrt(2) * B_i + Bq_A[2] / np.sqrt(2) * B_i
        )
        Es_A[ii, :] = np.sort(Es_A[ii, :])
    ...
    qind = 2
    print(
        "F_g = 1: F_e = 0: {0:.3f} F_e = 1: {1:.3f}".format(
            np.sum(dijq[qind, 0:3, 0] ** 2), np.sum(dijq[qind, 0:3, 1::] ** 2)
        )
    )
    print(
        "F_g = 0: F_e = 0: {0:.3f} F_e = 1: {1:.3f}".format(
    ...
    print(
        "F_g = 2: F_e = 0: {0:.3f} F_e = 1: {1:.3f}".format(
