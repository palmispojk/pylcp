"""Static plots of low-power blue MOT simulation results."""
import matplotlib
matplotlib.use('Agg')
import sys, os
...
results = load_results('low_power_blue_mot_simulation_data.pkl')
print(f"Loaded {len(results)} atoms")

plot_final_positions(
    results, constants.kmag_real,
    title='Sr88 Low-Power Blue MOT Cloud',
    filename='low_power_blue_mot_cloud_2d_xy.png',
)
plot_final_positions(
    results, constants.kmag_real,
    title='Sr88 Low-Power Blue MOT Cloud (beam axis)',
    filename='low_power_blue_mot_cloud_2d_xz.png',
    axes='xz',
)

plot_trajectories(
    results, constants.alpha_nat,
    filename='low_power_blue_mot_3x2_trajectories.png',
)
...
plot_distributions(
    dist_fits,
    title='Sr88 Low-Power Blue MOT — Captured Atom Distributions',
    filename='low_power_blue_mot_distributions.png',
)

T_data = temperature_vs_time(results, units, mask=mask)
plot_temperature_vs_time(
    T_data,
    title='Sr88 Low-Power Blue MOT — Temperature vs Time',
    filename='low_power_blue_mot_temperature.png',
