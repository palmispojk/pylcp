    ...
    def test_padding_when_not_divisible(self):
        ...
        if padded_N > N:
            np.testing.assert_array_equal(
                np.array(out[N:]), np.zeros((padded_N - N, 2))
            )
...
class TestMultiDeviceDense:
    ...
    def test_exponential_decay_accuracy(self):
        ...
        for i in range(N):
            np.testing.assert_allclose(
                np.array(ys[i, :, 0]), np.array(expected), atol=1e-4,
                err_msg=f"Atom {i} failed"
            )
    ...
    def test_matches_single_device(self):
        ...
        with mock.patch.object(itg, '_gpu_devices', return_value=[]):
            ts_ref, ys_ref = solve_ivp_dense(exp_decay, [0., 1.], y0,
                                             n_points=n_points)

        # Multi-device (autouse fixture active).
        ts_shard, ys_shard = solve_ivp_dense(exp_decay, [0., 1.], y0,
                                             n_points=n_points)
        np.testing.assert_allclose(np.array(ts_ref), np.array(ts_shard),
                                   atol=1e-12)
        np.testing.assert_allclose(np.array(ys_ref), np.array(ys_shard),
                                   atol=1e-10)
    ...
    def test_uneven_batch_matches(self):
        ...
        N = N_DEVICES * 2 + 1  # odd
        y0 = jnp.ones((N, 1)) * 3.0
        ts, ys = solve_ivp_dense(exp_decay, [0., 1.], y0, n_points=21)
    ...
    def test_harmonic_energy_conservation(self):
        ...
        for i in range(N):
            ...
            np.testing.assert_allclose(
                np.array(energy), 0.5, atol=1e-5,
                err_msg=f"Energy not conserved for atom {i}"
            )
...
class TestMultiDeviceRandom:
    ...
    def test_matches_single_device(self):
        ...
        for i in range(N):
            np.testing.assert_allclose(
                np.array(sols_ref[i].y), np.array(sols_shard[i].y),
                atol=1e-10,
